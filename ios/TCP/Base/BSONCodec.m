...
+ (id) BSONFragment: (NSData *) data at: (const void **) base ofType: (uint8_t) t
{
	...
	while (current < endPoint - 1)
	{
		...
		*base = current;
        ...
        // objc_msgSend兼容64位 add by kucky 

        value = ((id(*)(id,SEL,NSData *,const void **,uint8_t))objc_msgSend)(typeClass, @selector(BSONFragment:at:ofType:), data, base, typeID);
...
- (uint8_t) BSONTypeID
{
	...
	switch (encoding) {
		...
		case 'q': return 0x12;
		default:
			[NSException raise: NSInvalidArgumentException format: @"%@::%s - invalid encoding type '%c'", [self class], sel_getName(_cmd), encoding];
	}
	return 0;
}
...
+ (id) BSONFragment: (NSData *) data at: (const void **) base ofType: (uint8_t) t
{
	...
	{
        int32_t value;
        my_memcpy(&value, *base, sizeof(int32_t));
		value = BSONTOHOST32(value);
		*base += 4;
	...
	{
        int64_t value;
        my_memcpy(&value, *base, sizeof(int64_t));
		value = BSONTOHOST64(value);
		*base += 8;
