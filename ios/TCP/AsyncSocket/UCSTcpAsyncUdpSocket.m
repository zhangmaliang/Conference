...
NSString *const UCSTcpAsyncUdpSocketException = @"UCSTcpAsyncUdpSocketException";
NSString *const UCSTcpAsyncUdpSocketErrorDomain = @"UCSTcpAsyncUdpSocketErrorDomain";

#if MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_5
//   x lock used by all instances of UCSTcpAsyncUdpSocket, to protect getaddrinfo.
// Prior to Mac OS X 10.5 this method was not thread-safe.
static NSString *getaddrinfoLock = @"lock";
#endif

enum UCSTcpAsyncUdpSocketFlags
{
...
@interface UCSTcpAsyncUdpSocket (Private)
...
/**
 * The UCSAsyncSendPacket encompasses the instructions for a single send/write.
**/
@interface UCSAsyncSendPacket : NSObject
{
@public
	NSData *buffer;
	NSData *address;
...
@implementation UCSAsyncSendPacket
...
/**
 * The UCSAsyncReceivePacket encompasses the instructions for a single receive/read.
**/
@interface UCSAsyncReceivePacket : NSObject
{
@public
	NSTimeInterval timeout;
	long tag;
...
@implementation UCSAsyncReceivePacket
...
@implementation UCSTcpAsyncUdpSocket
...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
	...
	{
		[NSException raise:UCSTcpAsyncUdpSocketException
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	if(errPtr)
	{
		...
		*errPtr = [NSError errorWithDomain:UCSTcpAsyncUdpSocketErrorDomain
									  code:UCSTcpAsyncUdpSocketBadParameter
								  userInfo:info];
	}
	return NO;
}
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:UCSTcpAsyncUdpSocketErrorDomain code:UCSTcpAsyncUdpSocketCFSocketError userInfo:info];
}
...
- (NSError *)getIPv4UnavailableError
{
	...
	return [NSError errorWithDomain:UCSTcpAsyncUdpSocketErrorDomain code:UCSTcpAsyncUdpSocketIPv4Unavailable userInfo:info];
}
...
- (NSError *)getIPv6UnavailableError
{
	...
	return [NSError errorWithDomain:UCSTcpAsyncUdpSocketErrorDomain code:UCSTcpAsyncUdpSocketIPv6Unavailable userInfo:info];
}
...
- (NSError *)getSendTimeoutError
{
	...
	return [NSError errorWithDomain:UCSTcpAsyncUdpSocketErrorDomain code:UCSTcpAsyncUdpSocketSendTimeoutError userInfo:info];
}
- (NSError *)getReceiveTimeoutError
{
	...
	return [NSError errorWithDomain:UCSTcpAsyncUdpSocketErrorDomain code:UCSTcpAsyncUdpSocketReceiveTimeoutError userInfo:info];
}
...
- (BOOL)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	UCSAsyncSendPacket *packet = [[UCSAsyncSendPacket alloc] initWithData:data address:nil timeout:timeout tag:tag];
...
- (BOOL)sendData:(NSData *)data toHost:(NSString *)host port:(UInt16)port withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	UCSAsyncSendPacket *packet = nil;
	
	if(address4 && theSocket4)
		packet = [[UCSAsyncSendPacket alloc] initWithData:data address:address4 timeout:timeout tag:tag];
	else if(address6 && theSocket6)
		packet = [[UCSAsyncSendPacket alloc] initWithData:data address:address6 timeout:timeout tag:tag];
	else
		return NO;
...
- (BOOL)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	UCSAsyncSendPacket *packet = [[UCSAsyncSendPacket alloc] initWithData:data address:remoteAddr timeout:timeout tag:tag];
...
- (CFSocketRef)socketForPacket:(UCSAsyncSendPacket *)packet
{
	if(!theSocket4)
		return theSocket6;
	if(!theSocket6)
...
- (void)receiveWithTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	UCSAsyncReceivePacket *packet = [[UCSAsyncReceivePacket alloc] initWithTimeout:timeout tag:tag];
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketWriteCallBack:
			...
			break;
		default:
			NSLog (@"UCSTcpAsyncUdpSocket %p received unexpected CFSocketCallBackType %lu.", self, (unsigned long)type);
			break;
	}
}
...
static void MyCFSocketCallback(CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	UCSTcpAsyncUdpSocket *theSocket = [[(UCSTcpAsyncUdpSocket *)pInfo retain] autorelease];
