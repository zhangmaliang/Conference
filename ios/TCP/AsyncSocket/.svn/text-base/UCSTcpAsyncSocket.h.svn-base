...
@class UCSTcpAsyncSocket;
@class UCSAsyncReadPacket;
@class UCSAsyncWritePacket;

extern NSString *const UCSTcpAsyncSocketException;
extern NSString *const UCSTcpAsyncSocketErrorDomain;

enum UCSTcpAsyncSocketError
{
	UCSTcpAsyncSocketCFSocketError = kCFSocketError,	// From CFSocketError enum.
	UCSTcpAsyncSocketNoError = 0,						// Never used.
	UCSTcpAsyncSocketCanceledError,					// onSocketWillConnect: returned NO.
	UCSTcpAsyncSocketConnectTimeoutError,
	UCSTcpAsyncSocketReadMaxedOutError,               // Reached set maxLength without completing
	UCSTcpAsyncSocketReadTimeoutError,
	UCSTcpAsyncSocketWriteTimeoutError
};
typedef enum UCSTcpAsyncSocketError UCSTcpAsyncSocketError;

@protocol UCSTcpAsyncSocketDelegate
@optional
...
**/
- (void)onSocket:(UCSTcpAsyncSocket *)sock didAcceptNewSocket:(UCSTcpAsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(UCSTcpAsyncSocket *)sock wantsRunLoopForNewSocket:(UCSTcpAsyncSocket *)newSocket;

/**
 * Called when a socket is about to connect. This method should return YES to continue, or NO to abort.
 * If aborted, will result in UCSTcpAsyncSocketCanceledError.
 * 
...
**/
- (BOOL)onSocketWillConnect:(UCSTcpAsyncSocket *)sock;
...
**/
- (void)onSocket:(UCSTcpAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(UCSTcpAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(UCSTcpAsyncSocket *)sock;
...
@interface UCSTcpAsyncSocket : NSObject
{
	...
	UCSAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	UCSAsyncWritePacket *theCurrentWrite;
...
// Once one of the accept or connect methods are called, the UCSTcpAsyncSocket instance is locked in
...
// When an incoming connection is accepted, UCSTcpAsyncSocket invokes several delegate methods.
 ...
 * When a connection comes in, the UCSTcpAsyncSocket instance will call the various delegate methods (see above).
 ...
 * 
 * Please note the recommended way of releasing an UCSTcpAsyncSocket instance (e.g. in a dealloc method)
 * [UCSTcpAsyncSocket setDelegate:nil];
 * [UCSTcpAsyncSocket disconnect];
 * [UCSTcpAsyncSocket release];
**/
- (void)disconnect;
...
// The readData and writeData methods won't block (they are UCSAsynchronous).
// 
 ...
 * 
 * If you pass a buffer, you must not alter it in any way while UCSTcpAsyncSocket is using it.
 ...
 * it is treated similarly to a timeout - the socket is closed with a UCSTcpAsyncSocketReadMaxedOutError.
 ...
 * The X509Certificate class is part of the CocoaUCSTcpAsyncSocket open source project.
**/
- (void)startTLS:(NSDictionary *)tlsSettings;

/**
 * For handling readDataToData requests, data is necessarily read from the socket in small increments.
 * The performance can be much improved by allowing UCSTcpAsyncSocket to read larger chunks at a time and
...
/**
 * When you create an UCSTcpAsyncSocket, it is added to the runloop of the current thread.
...
/**
 * Returns the current run loop modes the UCSTcpAsyncSocket instance is operating in.
 * The default set of run loop modes is NSDefaultRunLoopMode.
**/
