...
//
//  http://code.google.com/p/cocoaUCSTcpAsyncSocket/
//
...
// UCSTcpAsyncSocket is RunLoop based, and is thus not thread-safe.
// You must always access your UCSTcpAsyncSocket instance from the thread/runloop in which the instance is running.
...
// then you may consider using GCDUCSTcpAsyncSocket instead, which is thread-safe.

NSString *const UCSTcpAsyncSocketException = @"UCSTcpAsyncSocketException";
NSString *const UCSTcpAsyncSocketErrorDomain = @"UCSTcpAsyncSocketErrorDomain";


enum UCSTcpAsyncSocketFlags
{
...
@interface UCSTcpAsyncSocket (Private)
...
/**
 * The UCSAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface UCSAsyncReadPacket : NSObject
{
  @public
...
@implementation UCSAsyncReadPacket
...
/**
 * The UCSAsyncWritePacket encompasses the instructions for any given write.
**/
@interface UCSAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation UCSAsyncWritePacket
...
/**
 * The UCSAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface UCSAsyncSpecialPacket : NSObject
{
  @public
...
@implementation UCSAsyncSpecialPacket
...
@implementation UCSTcpAsyncSocket
...
- (id)initWithDelegate:(id)delegate usingVoip:(BOOL)isVoip
{
...
- (void)checkForThreadSafety
{
	...
	{
		// UCSTcpAsyncSocket is RunLoop based.
		...
		// If you are seeing errors or crashes in UCSTcpAsyncSocket,
		...
		// you may prefer to use GCDUCSTcpAsyncSocket which is thread-safe.
		
		[NSException raise:UCSTcpAsyncSocketException
		            format:@"Attempting to access UCSTcpAsyncSocket instance from incorrect thread."];
	}
}
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	// and that the read packet isn't an UCSAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[UCSAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	// and that the write packet isn't an UCSAsyncSpecialPacket (upgrade to TLS).
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[UCSAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	else
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketCFSocketError userInfo:info];
		}
		return NO;
	}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		// Note: We use [self class] to support subclassing UCSTcpAsyncSocket.
		UCSTcpAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		NSLog(@"UCSTcpAsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
							   ...
							   (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"UCSTcpAsyncSocket %p couldn't attach read stream to run-loop,", self);
		NSLog (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext)))
	{
		...
		NSLog (@"UCSTcpAsyncSocket %p couldn't attach write stream to run-loop,", self);
		NSLog (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		NSLog (@"UCSTcpAsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen(theWriteStream))
	{
		NSLog (@"UCSTcpAsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
 ...
 * When the socket is connected and both streams are open, consider the UCSTcpAsyncSocket instance to be ready.
**/
- (void)doStreamOpen
{
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
		...
		{
			NSLog (@"UCSTcpAsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(peeraddr == NULL)
	{
		NSLog(@"UCSTcpAsyncSocket couldn't determine IP version of socket");
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		// Check to see if it's a normal read packet (not UCSAsyncSpecialPacket) and if it had read anything yet.
		
		if(([theCurrentRead isKindOfClass:[UCSAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"UCSTcpAsyncSocketCFSocketError",
														 @"UCSTcpAsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketCFSocketError userInfo:info];
}
...
/**
 * Returns a standard UCSTcpAsyncSocket abort error.
**/
- (NSError *)getAbortError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"UCSTcpAsyncSocketCanceledError",
														 @"UCSTcpAsyncSocket", [NSBundle mainBundle],
														 @"Connection canceled", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketCanceledError userInfo:info];
}

/**
 * Returns a standard UCSTcpAsyncSocket connect timeout error.
**/
- (NSError *)getConnectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"UCSTcpAsyncSocketConnectTimeoutError",
														 @"UCSTcpAsyncSocket", [NSBundle mainBundle],
														 @"Attempt to connect to host timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketConnectTimeoutError userInfo:info];
}

/**
 * Returns a standard UCSTcpAsyncSocket maxed out error.
**/
- (NSError *)getReadMaxedOutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"UCSTcpAsyncSocketReadMaxedOutError",
														 @"UCSTcpAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketReadMaxedOutError userInfo:info];
}

/**
 * Returns a standard UCSTcpAsyncSocket read timeout error.
**/
- (NSError *)getReadTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"UCSTcpAsyncSocketReadTimeoutError",
														 @"UCSTcpAsyncSocket", [NSBundle mainBundle],
														 @"Read operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketReadTimeoutError userInfo:info];
}

/**
 * Returns a standard UCSTcpAsyncSocket write timeout error.
**/
- (NSError *)getWriteTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"UCSTcpAsyncSocketWriteTimeoutError",
														 @"UCSTcpAsyncSocket", [NSBundle mainBundle],
														 @"Write operation timed out", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UCSTcpAsyncSocketErrorDomain code:UCSTcpAsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	[ms appendString:[NSString stringWithFormat:@"<UCSTcpAsyncSocket %p", self]];
	...
	if (theCurrentRead == nil || [theCurrentRead isKindOfClass:[UCSAsyncSpecialPacket class]])
	...
	if (theCurrentWrite == nil || [theCurrentWrite isKindOfClass:[UCSAsyncSpecialPacket class]])
                        ...
                        tag:(long)tag
{
	...
	UCSAsyncReadPacket *packet = [[UCSAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[UCSAsyncSpecialPacket class]])
			{
...
- (void)doBytesAvailable
{
	...
	// Note: This method is not called if theCurrentRead is an UCSAsyncSpecialPacket (startTLS packet)
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	UCSAsyncWritePacket *packet = [[UCSAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[UCSAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)doSendBytes
{
	...
	// Note: This method is not called if theCurrentWrite is an UCSAsyncSpecialPacket (startTLS packet)
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	UCSAsyncSpecialPacket *packet = [[UCSAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	{
		UCSAsyncSpecialPacket *tlsPacket = (UCSAsyncSpecialPacket *)theCurrentRead;
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog(@"UCSTcpAsyncSocket %p received unexpected CFSocketCallBackType %i", self, (int)type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog(@"UCSTcpAsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog(@"UCSTcpAsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %i", self, (int)type);
	}
}
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	UCSTcpAsyncSocket *theSocket = [[(UCSTcpAsyncSocket *)pInfo retain] autorelease];
