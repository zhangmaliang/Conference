...
@class UCSAsyncSendPacket;
@class UCSAsyncReceivePacket;

extern NSString *const UCSTcpAsyncUdpSocketException;
extern NSString *const UCSTcpAsyncUdpSocketErrorDomain;

enum UCSTcpAsyncUdpSocketError
{
	UCSTcpAsyncUdpSocketCFSocketError = kCFSocketError,	// From CFSocketError enum
	UCSTcpAsyncUdpSocketNoError = 0,						// Never used
	UCSTcpAsyncUdpSocketBadParameter,                     // Used if given a bad parameter (such as an improper address)
	UCSTcpAsyncUdpSocketIPv4Unavailable,                  // Used if you bind/connect using IPv6 only
	UCSTcpAsyncUdpSocketIPv6Unavailable,                  // Used if you bind/connect using IPv4 only (or iPhone)
	UCSTcpAsyncUdpSocketSendTimeoutError,
	UCSTcpAsyncUdpSocketReceiveTimeoutError
};
typedef enum UCSTcpAsyncUdpSocketError UCSTcpAsyncUdpSocketError;

@interface UCSTcpAsyncUdpSocket : NSObject
{
	...
	UCSAsyncSendPacket *theCurrentSend;
	NSTimer *theSendTimer;
	
	NSMutableArray *theReceiveQueue;
	UCSAsyncReceivePacket *theCurrentReceive;
...
/**
 * Creates new instances of UCSTcpAsyncUdpSocket.
**/
- (id)init;
...
/**
 * Creates new instances of UCSTcpAsyncUdpSocket that support only IPv4 or IPv6.
...
/**
 * UCSAsynchronously sends the given data, with the given timeout and tag.
 * 
...
/**
 * UCSAsynchronously sends the given data, with the given timeout and tag, to the given host and port.
 * 
...
/**
 * UCSAsynchronously sends the given data, with the given timeout and tag, to the given address.
 * 
 * This method cannot be used with a connected socket.
 * 
 * If data is nil or zero-length, this method does nothing and immediately returns NO.
...
/**
 * UCSAsynchronously receives a single datagram packet.
 * 
...
/**
 * When you create an UCSTcpAsyncUdpSocket, it is added to the runloop of the current thread.
...
@protocol UCSTcpAsyncUdpSocketDelegate
@optional

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)onUdpSocket:(UCSTcpAsyncUdpSocket *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)onUdpSocket:(UCSTcpAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error;
 ...
 * and UCSTcpAsyncUdpSocket will continue as if the packet never arrived.
...
**/
- (BOOL)onUdpSocket:(UCSTcpAsyncUdpSocket *)sock didReceiveData:(NSData *)data withTag:(long)tag fromHost:(NSString *)host port:(UInt16)port;
...
**/
- (void)onUdpSocket:(UCSTcpAsyncUdpSocket *)sock didNotReceiveDataWithTag:(long)tag dueToError:(NSError *)error;

/**
 * Called when the socket is closed.
 * A socket is only closed if you explicitly call one of the close methods.
**/
- (void)onUdpSocketDidClose:(UCSTcpAsyncUdpSocket *)sock;
